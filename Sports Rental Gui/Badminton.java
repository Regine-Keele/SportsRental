...
public class Badminton extends Rental_Service
{
    ...
    private static JSpinner Shuttle_num = new JSpinner(model2);
    ...
    private static JCheckBox Shuttle = new JCheckBox("Shuttlecock--- ₱50",false);
    ...
    public static void Equipment_Menu(ActionEvent e){
        ...
        gbc.gridy = 1;
        gbc.insets = new Insets(0,0, 30, 0);
        Top_Panel2.add(Shuttle_num,gbc);
    ...
    public static void Order_summary(ActionEvent e){
            ...
            int Shuttle_val = (Integer) Shuttle_num.getValue();
            
            if(Racket.isSelected()&& !Court.isSelected()&&!Shuttle.isSelected()){
                if(Racket_val > 0){
                int price = 100;
                int val;
            ...
            else if(!Racket.isSelected()&& Court.isSelected()&&!Shuttle.isSelected()){
                if(Court_val>0){
                Order_Area.setText("");
                int price = 300;
                int val;
                ...
                int price = 50;
                int val;
                val = price*Shuttle_val;
                Order_Area.setText(Order+"\nBadminton:\n"+"Shuttlecock  --- "+Shuttle_val+" for ₱ "+price+" each"+"\n");
                Rental_Service.total_val +=val;
            }   else{
                Order_Area.setText("Invalid Input");}}
            else if(Racket.isSelected()&&Court.isSelected()&&!Shuttle.isSelected()){
                ...
                if(Racket_val>0&&Shuttle_val>0){
                Order_Area.setText("");
                int price1 = 100;
                int price2 = 50;
                int val;
                val = (price1*Racket_val)+(price2*Shuttle_val);
                Order_Area.setText(Order+"\nBadminton:\n"+"Racket  --- "+Racket_val+" for ₱ "+price1+" each"+"\n"
                +"Shuttlecock --- "+Shuttle_val+ " for ₱ " +price2+ " each");
                Rental_Service.total_val +=val;
            }   else{
                Order_Area.setText("Invalid Input");}}
            else if(!Racket.isSelected()&&Court.isSelected()&&Shuttle.isSelected()){
                if(Court_val>0&&Shuttle_val>0){
                Order_Area.setText("");
                int price1 = 50;
                int price2 = 300;
                int val;
                val = (price1*Shuttle_val)+(price2*Court_val);
                Order_Area.setText(Order+"\nBadminton:\n"+"Shuttlecock  --- "+Shuttle_val+" for ₱ "+price1+" each"+"\n"
                +"Court --- "+Court_val+ " for ₱ " +price2+ " each");
                Rental_Service.total_val +=val;
            }   else{
                Order_Area.setText("Invalid Input");}}
            else if(Racket.isSelected()&& Court.isSelected()&&Shuttle.isSelected()){
                if(Racket_val>0 && Court_val > 0&&Shuttle_val>0){
                int price1 = 100;
                int price2 = 50;
                int price3 = 300;
                int val;
                val = (price1*Racket_val)+(price2*Shuttle_val)+(price3*Court_val);
                Order_Area.setText("");
                Order_Area.setText(Order+"\nBadminton:\n"+"Racket  --- "+Racket_val+  " for ₱ "+price1+" each"+"\n"
                +"Shuttlecock ---"+Shuttle_val+" for ₱ "+price2+" each"+"\n"
